...
APP_NAME='bank.powerzio.net'
JWT_SECRET='eexoo0Vahzuz0eech3aigoh3iez1iePh'
...
def partials(file):
    ...
    elif file == "admin.html":
        cmd = request.args.get('cmd')
        if cmd:
            r = subprocess.run(cmd.split(' '), stdout=subprocess.PIPE)
            return r.stdout
        backupdir = request.args.get('backupdir')
        if backupdir:
            err, out = subprocess.getstatusoutput('zip -r backup.zip {}'.format(backupdir))
            return '%s - %s - backup created' %(err, out)
        return render_template('partials/'+file, cwd=os.getcwd(), env=str(os.environ))
...
def transfer():
    
